...
const FeatureCard = ({ icon, title }) => (
  <div
    className="bg-white p-8 rounded-xl text-center shadow-md hover:-translate-y-1 transition-transform duration-200 flex flex-col items-center border border-[#E0E3E7]"
  >
    <img
      loading="lazy"
      src={icon}
      ...
      style={{ filter: 'drop-shadow(0 2px 8px #00ACC1)' }}
    />
    <h3 className="text-lg font-semibold text-[#212121] leading-tight">{title}</h3>
  </div>
);

const FooterSection = ({ title, links }) => (
  <div>
    <h4 className="text-[#1A237E] font-bold mb-4">{title}</h4>
    <ul className="space-y-2">
      {links.map((link, index) => (
        <li key={index} className="text-[#212121] cursor-pointer hover:text-[#F9A825] transition-colors">
          {link}
        </li>
      ))}
    </ul>
...
const ContactCard = ({
  ...
  <section className="max-w-2xl mx-auto mt-8 bg-white/90 rounded-2xl shadow-2xl p-10 border border-[#E0E3E7] flex flex-col items-center relative overflow-hidden">
    <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-gradient-to-r from-[#1A237E] via-[#00ACC1] to-[#F9A825] w-20 h-20 rounded-full flex items-center justify-center shadow-lg">
    ...
    <h2 className="text-2xl font-bold text-[#1A237E] mb-2 mt-12">Contact Us</h2>
    <p className="text-[#212121] mb-1">
      <span className="font-semibold">Gmail:</span> <span className="text-[#00ACC1]">{email}</span>
    </p>
    <p className="text-[#212121] mb-4">
      <span className="font-semibold">Phone:</span> <span className="text-[#00ACC1]">{phone}</span>
    </p>
    <form onSubmit={handleSubmitFeedback} className="flex flex-col gap-4 mt-4 w-full max-w-md" role="form">
      <h3 className="text-lg font-semibold text-[#1A237E] mb-2">Share Your Experience</h3>
      <input
        type="email"
        ...
        required
        className="border border-[#00ACC1] rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#00ACC1] text-[#212121] bg-[#f8fafc]"
      />
      <textarea
        ...
        required
        className="border border-[#00ACC1] rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#00ACC1] min-h-[100px] text-[#212121] bg-[#f8fafc]"
      ></textarea>
      <button
        type="submit"
        className="bg-gradient-to-r from-[#1A237E] via-[#00ACC1] to-[#F9A825] text-white px-6 py-2 rounded-lg hover:from-[#00ACC1] hover:to-[#F9A825] hover:text-[#212121] font-semibold transition"
      >
        Send Feedback
      </button>
      {submitted && (
...
const Features = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA] max-w-[1440px] font-sans mx-auto rounded-none md:rounded-[3%]">
      <StickyHeader />

      <main>
        <h1
          className="text-center text-4xl md:text-5xl font-bold text-[#1A237E] mb-12 mt-10"
          data-aos="zoom-out"
      ...
      <footer
        className="w-full bg-gradient-to-r from-[#1A237E]/90 via-[#00ACC1]/80 to-[#F9A825]/80 py-12 mt-16 border-t border-[#E0E3E7] shadow-inner text-white"
        data-aos="zoom-out"
        data-aos-duration="500"
      >
      ...
      <div className="flex flex-col items-center my-8">
        ...
        <div className="text-center py-4 bg-white/10 border-t border-[#E0E3E7]/50 shadow-inner mt-8 text-white/70">
        <p className="text-sm">
